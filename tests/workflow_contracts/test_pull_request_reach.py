...
@pytest.mark.parametrize(
    ...
    [
        ...
        "leynos/falcon-pachinko/.github/workflows/called.yml@main",
    ],
)
...
def test_a_pull_request_target_workflow_is_an_entry_point() -> None:
...
def test_a_folded_guard_splits_into_its_conjuncts() -> None:
    ...
    assert conjuncts(condition) == ["github.ref == 'refs/heads/main'", "env.T != ''"], (
        f"{condition!r} must split into its two conjuncts"
    )
...
def test_a_guard_that_is_not_a_conjunction_is_refused(condition: str) -> None:
    ...
    Appended to a guard it makes every conjunct optional while a substring
    check still finds the ref clause it was looking for.
    """
