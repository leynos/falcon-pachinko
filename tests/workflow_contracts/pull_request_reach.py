...
holds. The hole was measured elsewhere in this estate, not reasoned about: a
``workflow_call``-only workflow, called from a pull-request job with
``secrets: inherit`` and curling the CodeScene API with the inherited token,
passed every contract that enumerated triggers alone.
...
See Also
--------
codescene_scan.pull_request_workflows : The closure over this repository's
    workflows.
"""
...
REPOSITORY: typ.Final[str] = "leynos/falcon-pachinko"
