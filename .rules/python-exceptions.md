...
## 2) Raise the right thing, with the right cause (TRY002/TRY004/TRY201)
...
retains traceback lineage. Avoid discarding causes in contexts expected to
preserve them.
...
## 4) Message construction for raises (TRY003) and logging practice (LOG rules, TRY401)

### Exception messages: construct once, pass once

```python
name = user.name
# ❌ f-string passed directly into the constructor (TRY003 flags long
# messages built at the raise site)
...
# ❌ LOG issues
logging.warning(f"failed for {user_id}")  # f-string
logging.warning("failed for %s" % user_id)  # %-formatting
logging.warn("deprecated")  # warn()
logging.error("bad root logger")  # root logger usage
...
The Pylint gate reinforces this with the `logging-*` message family
(`logging-fstring-interpolation`, `logging-not-lazy`, and friends).
...
try:
    ...
    logger.exception("Risky operation failed")  # ✅ traceback included
```
...
def charge(amount_pennies: int, card_token: str) -> str:
    try:
        ...
        msg = "Gateway timeout"
        raise PaymentsError(msg) from exc  # ✅ TRY201
...
def worker_main() -> None:
    try:
        ...
        logger.exception("Job failed due to payments error")  # log once
        raise
```

### Building exception messages and logging payloads
...
logger.info("Dispatching order_id=%s to shop_id=%s", order_id, shop_id)
```
...
> messages directly; log exceptions once at a boundary via
> `logger.exception`. Enforce with Ruff (TRY/BLE/LOG/N818/PERF203/B017).

## 11) References

- Ruff rules: Tryceratops (TRY), Blind Except (BLE001), flake8-errmsg
  (EM101/EM102), flake8-logging (LOG), N818, PERF203, B017.
