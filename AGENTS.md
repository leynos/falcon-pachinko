...
- [Python Exception Design](.rules/python-exceptions.md) - Exception
  hierarchies, raising, handling, and logging
...
- The spelling configuration `typos.toml` is generated by the shared
  `typos-config-builder` CLI. Edit `typos.local.toml` only for narrow
  repository terminology, then run `make spelling-config-write`; never edit
